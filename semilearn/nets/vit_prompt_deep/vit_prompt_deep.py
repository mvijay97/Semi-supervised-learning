...
class DeepPromptedVisionTransformer(nn.Module):
    """ Vision Transformer
    ...
    def __init__(
        ...
        # Shallow prompt embedding
        self.prompt_embeddings = nn.Parameter(torch.zeros(1, self.num_prompt_tokens, self.prompt_dim)) # train
        nn.init.uniform_(self.prompt_embeddings.data, -val, val)

        # Deep prompt embedding
        total_d_layer = depth - 1
        self.deep_prompt_embeddings = nn.Parameter(torch.zeros(total_d_layer, self.num_prompt_tokens, self.prompt_dim))
        ...
        self.pos_embed = nn.Parameter(torch.zeros(1, num_patches + self.num_tokens, embed_dim)) # check this
    ...
    def incorporate_prompt(self, x):
        ...
        x_prompt = self.prompt_proj(self.prompt_embeddings).expand(B, -1, -1)
        x = torch.cat(
            (
                x[:, :1, :],
                x_prompt,
...
def deep_prompted_vit_tiny_patch2_32(pretrained=False, pretrained_path=None, **kwargs):
    """ ViT-Tiny (Vit-Ti/2)
    """
...
def deep_prompted_vit_small_patch2_32(pretrained=False, pretrained_path=None, **kwargs):
    """ ViT-Small (ViT-S/2)
    """
...
def deep_prompted_vit_small_patch16_224(pretrained=False, pretrained_path=None, **kwargs):
    """ ViT-Small (ViT-S/16)
    """
...
def deep_prompted_vit_base_patch16_96(pretrained=False, pretrained_path=None, **kwargs):
...
def deep_prompted_vit_base_patch16_224(pretrained=False, pretrained_path=None, **kwargs):
    """ ViT-Base (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k @ 224x224, source https://github.com/google-research/vision_transformer.
    """
